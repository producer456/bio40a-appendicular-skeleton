...
const ISCHIUM = ['Ischial spine', 'Ischial tuberosity', 'Lesser sciatic notch'];
const CARPALS = [
...
const IMAGE_DATA = {
    ...
    radius_ulna: {
        ...
        words: [
            'Radius', 'Ulna',
            'Head of radius', 'Neck of radius', 'Radial tuberosity', 'Styloid process of radius',
            'Olecranon', 'Trochlear notch', 'Coronoid process of ulna',
    ...
    hand: {
        ...
        words: [
            ...CARPALS,
            'Metacarpal I', 'Metacarpal II', 'Metacarpal III',
    ...
    foot: {
        ...
        words: [
            ...TARSALS,
            'Metatarsal I', 'Metatarsal II', 'Metatarsal III',
            'Metatarsal IV', 'Metatarsal V',
            'Proximal phalanx', 'Middle phalanx', 'Distal phalanx'
        ]
    },
    lab_packet: {
...
function saveMarkers() {
    try {
        localStorage.setItem('bio40a_appendicular_markers', JSON.stringify(markers));
        localStorage.setItem('bio40a_appendicular_markerCounter', markerIdCounter);
    } catch(e) {}
}

function loadMarkers() {
    try {
        const data = localStorage.getItem('bio40a_appendicular_markers');
        if (data) markers = JSON.parse(data);
        const counter = localStorage.getItem('bio40a_appendicular_markerCounter');
...
function loadAnswerKeys() {
    try {
        const data = localStorage.getItem('bio40a_appendicular_answerKeys');
...
function exportData() {
    const payload = {
        app: 'bio40a-appendicular-skeleton',
        version: 1,
...
function importData(event) {
    ...
    reader.onload = (e) => {
        try {
            ...
            if (payload.app !== 'bio40a-appendicular-skeleton') {
                showToast('Not a BIO 40A appendicular backup file', 'error');
                return;
            }
            const ok = confirm(
